...
class SceneJobPixel : public pr::Job {
		int x;
		int y;
		Scene &scene;
		pr::Barrier &barrier;
		...
		public:
			SceneJobPixel(int x, int y, Scene &scene,pr::Barrier &b,  Color * pixels, vector<Vec3D> &lights)
				: x(x), y(y), scene(scene), barrier(b),pixels(pixels), lights(lights){}
